...
\multirow{8}{*}{Battery (BESS)}
  ...
  & Charge efficiency $\eta_c$ &   98 & \%        & DEA 2030 \\
  & Discharge efficiency $\eta_d$ & 97 & \%       & DEA 2030 \\
\midrule
...
$\eta_c,\,\eta_d$ & — & DC charge / discharge efficiency ($0.98$ / $0.97$) \\
...
\begin{equation}
  s_t = s_{t-1} + \eta_c\, u_t - \frac{d_t}{\eta_d}
  \quad \forall\, t
  \label{eq:soc}
\end{equation}

where $u_t$ and $d_t$ are the AC-side charge and discharge flows (matching the
energy balance), so that $\eta_c$ and $\eta_d$ capture only the DC cell-level
losses. The solar capacity factors are measured at the grid metering point
(AC output of the inverter), so inverter losses are already embedded in the
input data and must not be double-counted here.

\textbf{Cyclic initial condition.} Rather than assuming $s_{-1} = 0$, the SOC
recurrence uses a cyclic L matrix with $L_{0,\,T-1} = -1$. This makes the
implicit prior SOC at $t = 0$ equal to $s_{T-1}$ (the end-of-year state),
enforcing a self-consistent annual cycle: the battery neither accumulates nor
drains net energy across the year. In matrix form, the t=0 row reads
$s_0 - s_{T-1} = \eta_c u_0 - d_0/\eta_d$ rather than $s_0 = \eta_c u_0 -
d_0/\eta_d$. For typical Danish weather data the constraint is nearly
non-binding (the battery naturally empties by year-end), but it is correct
by construction.
...
promotes $E_b$ to a fourth free variable with its own capex term. Round-trip
efficiency losses are modelled in the SOC recurrence \eqref{eq:soc}; the
bounds here apply to the AC-side flows.
